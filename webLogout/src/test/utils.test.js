...
describe("getCxType", () => {
  it("should return 'PA' when token has organization", () => {
    const token = { organization: { id: "123", role: "admin" } };
    const result = getCxType(token);
    expect(result).toEqual("PA");
  });

  it("should return 'PG' when token.organization.roles[0].role starts with 'pg-'", () => {
    const token = { organization: { role: "pg-manager" } };
    const result = getCxType(token);
    expect(result).toEqual("PG");
  });

  it("should return 'PF' when token has no organization", () => {
...
describe("getCxId", () => {
  it("should return organization id if present", () => {
    const token = { organization: { id: "org-123" }, uid: "user-1" };
    const result = getCxId(token);
    expect(result).toEqual("org-123");
  });

  it("should return 'PF-' + uid if organization is missing", () => {
...
describe("getCxRole", () => {
  it("should return the role if present", () => {
    const token = { organization: { role: "admin" } };
    const result = getCxRole(token);
    expect(result).toEqual("admin");
  });
  it("should return undefined if organization is missing", () => {
    const token = {};
    const result = getCxRole(token);
    expect(result).toBeUndefined();
  });
  it("should return undefined if organization.role is missing", () => {
