...
describe('responses tests', () => {
  it("generateOkResponse", () => {
    const result = responses.generateOkResponse(sessionToken, decodedToken, origin)

    expect(result).to.eql(okResponse)
  })

  it("generateKoResponse Role not allowed", () => {
    const result = responses.generateKoResponse(new ValidationException('Role not allowed'), origin)
  ...
  it("generateKoResponse Invalid Audience", () => {
    const result = responses.generateKoResponse(new ValidationException('Invalid Audience'), origin)

    expect(result).to.eql(makeKoResponse('Invalid Audience', 400))
  })

  it("generateKoResponse Token is not valid", () => {
    const result = responses.generateKoResponse(new ValidationException('Token is not valid'), origin)
