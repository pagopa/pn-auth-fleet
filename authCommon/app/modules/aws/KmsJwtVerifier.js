...
async function validation(jwtToken, cacheTTL) {
  if (jwtToken) {
    const decodedToken = await jwtValidator(jwtToken, cacheTTL);
...
async function jwtValidator(jwtToken, cacheTTL) {
  ...
  if (cachedPublicKey) {
    ...
    setCachedData(keyId, decodedPublicKey, cacheTTL);
  }
  try {
    const publicKeyPem =
      "-----BEGIN PUBLIC KEY-----\n" +
...
function setCachedData(keyId, val, cacheTTL) {
